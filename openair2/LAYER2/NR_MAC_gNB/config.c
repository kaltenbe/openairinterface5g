...
static bool configure_pos_sib_schedule(gNB_MAC_INST *nrmac);
...
bool nr_mac_configure_other_sib(gNB_MAC_INST *nrmac, int num_cu_sib, const f1ap_sib_msg_t cu_sib[num_cu_sib])
{
  ...
  if (cc->pos_sib_bcch_length > 0 && !configure_pos_sib_schedule(nrmac))
    return false;
  return true;
}
...
bool nr_mac_configure_pos_sib(gNB_MAC_INST *nrmac, const NR_BCCH_DL_SCH_Message_t *pos_sib)
{
  ...
  if (sib1 && sib1->si_SchedulingInfo)
    activated = configure_pos_sib_schedule(nrmac);
